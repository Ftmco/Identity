<template>
  <v-col cols="12">
    ...
    >
      <template v-slot:top>
        <v-text-field v-model="search" label="Search Applications" class="mx-4">
      ...
      <template v-slot:item.image="{ item }">
        <v-img :src="item.image" :lazy-src="item.image" width="100" />
      </template>
      <template v-slot:item.actions="{ item }">
        <div>
          <v-row>
            <v-col>
              <v-btn outlined block color="red" @click="deleteApp(item)">
                <span>Delete</span>
                <v-icon>mdi-trash-can-outline</v-icon>
              </v-btn>
            </v-col>
            <v-col>
              <v-btn outlined block color="warning" @click="editApp(item)">
                <span>Edit</span>
    ...
    <Appdialog :title="dTitle" :titleColor="dColor">
      <template v-slot:body>
        <component @submit="submit" v-bind="props" :is="comp"> </component>
      </template>
    </Appdialog>
  </v-col>
</template>
    ...
    deleteConfirm(confirm: any) {
      if (confirm.agree) {
        this.applicationService
          .Delete(confirm.data)
          .then((res) => {
            if (res.status) this.removeItem(res.result.id);
